...
} from './_/utils/fileUploadUtil'
import { TestAppConfig } from './_/utils/types'
import { Popover, ListChoice, Button } from '@kiwicom/orbit-components'
...
export default class Editor extends PureComponent<ExampleEditorProps> {
  ...
  constructor(props: ExampleEditorProps) {
    ...
    const pluginsConfig = Plugins.getConfig(
      additionalConfig,
      props.shouldNativeUpload,
    )
    ...
    this.ricosPlugins = Object.entries(Plugins.ricosEditorPlugins).map(
      ([pluginType, plugin]) =>
        pluginType in pluginsConfig
          ? plugin(pluginsConfig[pluginType])
          : plugin(),
    )
  }
  ...
  initEditorProps() {
    ...
    this.helpers = {
      ...
      onOpenEditorSuccess: async (...args) =>
        console.log('onOpenEditorSuccess', ...args),
      onContentEdited: async (params) => console.log('onContentEdited', params),
      onToolbarButtonClick: async (params) =>
        console.log('onToolbarButtonClick', params),
      onPluginModalOpened: async (params) =>
        console.log('onPluginModalOpened', params),
      ...
      onVideoSelected: (url, updateEntity) => {
        ...
        setTimeout(() => {
          const mockVideoIndex =
            this.props.mockImageIndex ||
            Math.floor(Math.random() * testVideos.length)
  ...
  renderExternalToolbar() {
    const { externalToolbar: ExternalToolbar, externalToolbarToShow } =
      this.props
  ...
  render() {
    ...
    const textToolbarType: TextToolbarType =
      staticToolbar && !isMobile ? STATIC_TOOLBAR : null
    ...
    return (
      <div style={{ height: '100%' }}>
          ...
          <RicosEditor
            ...
            toolbarSettings={{
              ...
              textToolbarContainer:
                useStaticTextToolbar && this.staticToolbarContainer,
        ...
        <Popover
          ...
          width="250px"
        >
          <Button iconRight={<span>sdfs</span>} type="secondary">
            Open popover
          </Button>
        </Popover>
      </div>
