...
const { initElasticsearch, fixturesParams, isElasticsearchReady } =
  searchFixtures
...
test('[DSL] returns only assets with the specified asset types', async (t) => {
  ...
  t.truthy(
    filterDSLSearch.results.find((asset) => assetsIds.asset1 === asset.id),
  )
  t.truthy(
    filterDSLSearch.results.find((asset) => assetsIds.asset3 === asset.id),
  )
  ...
  t.truthy(
    filterDSLOverrideSearch.results.find(
      (asset) => assetsIds.asset1 === asset.id,
    ),
  )
  t.truthy(
    filterDSLOverrideSearch.results.find(
      (asset) => assetsIds.asset3 === asset.id,
    ),
  )
})
...
test('[DSL] filters assets based on active and validated parameters', async (t) => {
  ...
  t.falsy(
    filterDSLSearch.results.find((asset) => assetsIds.asset9 === asset.id),
  )
  ...
  t.falsy(
    filterDSLSearchShorthand.results.find(
      (asset) => assetsIds.asset9 === asset.id,
    ),
  )
  ...
  t.truthy(
    searchResults2.results.find((asset) => assetsIds.asset9 === asset.id),
  )
  ...
  t.truthy(
    filterDSLSearch2.results.find((asset) => assetsIds.asset9 === asset.id),
  )
  ...
  t.truthy(
    filterDSLSearch4.results.find((asset) => assetsIds.asset9 === asset.id),
  )
})
...
test('[DSL] returns assets with queried custom attributes (strict equality)', async (t) => {
  ...
  t.truthy(
    filterDSLSearch1.results.find((asset) => assetsIds.asset8 === asset.id),
  )
  ...
  t.truthy(
    filterDSLSearch2.results.find((asset) => assetsIds.asset7 === asset.id),
  )
  t.truthy(
    filterDSLSearch2.results.find((asset) => assetsIds.asset8 === asset.id),
  )
  ...
  t.truthy(
    filterDSLSearch3.results.find((asset) => assetsIds.asset7 === asset.id),
  )
})
...
test('[DSL] returns assets with queried custom attributes (range for numbers)', async (t) => {
  ...
  const {
    body: { results: mixedSyntaxSearch },
  } = await request(t.context.serverUrl)
    .post('/search')
    .set(authorizationHeaders)
    .send({
      // using both filter and customAttributes queries implies AND in ElasticSearch `filter` query
...
test('[DSL] returns assets with queried custom attributes of several types', async (t) => {
  ...
  t.truthy(
    filterDSLSearch.results.find((asset) => assetsIds.asset8 === asset.id),
  )
  ...
  const { body: mixedQuerySearch } = await request(t.context.serverUrl)
    ...
    .set(authorizationHeaders)
    .send({
      // filter and customAttributes always implicitly joined with AND
  ...
  t.truthy(
    mixedQuerySearch.results.find((asset) => assetsIds.asset8 === asset.id),
  )
})
...
test('[DSL] returns assets with queried custom attribute of "tags" type (one element match)', async (t) => {
  ...
  t.true(filterDSLSearch.results.length === 2)
  t.truthy(
    filterDSLSearch.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  t.truthy(
    filterDSLSearch.results.find((asset) => assetsIds.asset8 === asset.id),
  )
...
test('[DSL] returns assets with queried custom attribute of "select" type (match one of provided elements)', async (t) => {
  ...
  t.truthy(
    filterDSLSearch.results.find((asset) => assetsIds.asset7 === asset.id),
  )
  t.truthy(
    filterDSLSearch.results.find((asset) => assetsIds.asset8 === asset.id),
  )
...
test('[DSL] returns assets with queried custom attribute of "tags" type (match all, some or exact number of elements)', async (t) => {
  ...
  t.true(filterDSLSearch1.results.length === 1)
  t.truthy(
    filterDSLSearch1.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  ...
  t.truthy(
    filterDSLSearch1a.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  ...
  t.truthy(
    filterDSLSearch1b.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  ...
  t.true(filterDSLSearch2.results.length === 2)
  t.truthy(
    filterDSLSearch2.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  t.truthy(
    filterDSLSearch2.results.find((asset) => assetsIds.asset8 === asset.id),
  )
  ...
  t.truthy(
    filterDSLSearch4a.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  ...
  t.truthy(
    filterDSLSearch4b.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  t.truthy(
    filterDSLSearch4b.results.find((asset) => assetsIds.asset8 === asset.id),
  )
  ...
  t.truthy(
    filterDSLSearch4c.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  t.truthy(
    filterDSLSearch4c.results.find((asset) => assetsIds.asset8 === asset.id),
  )
  ...
  t.falsy(
    filterDSLSearch4d.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  t.truthy(
    filterDSLSearch4d.results.find((asset) => assetsIds.asset8 === asset.id),
  )
  t.truthy(
    filterDSLSearch4d.results.find((asset) => assetsIds.asset1 === asset.id),
  )
})
...
test('[DSL-only] returns assets that match exact price query', async (t) => {
  ...
  const { body: priceSearch3 } = await request(t.context.serverUrl)
    ...
    .send({
      filter: `_price == ${
        Number.isInteger(basePrice) ? `${basePrice}.` : basePrice
      }`,
...
test('[DSL-only] returns assets that match complex filter using built-ins', async (t) => {
  ...
  const { body: complexSearch } = await request(t.context.serverUrl)
    ...
    .set(authorizationHeaders)
    .send({
      filter: `(_price >= ${basePrice + 7} || _price < ${
        // useless price filter on purpose
        basePrice + 8
  ...
  t.truthy(
    complexSearch.results.find((asset) => assetsIds.asset13 === asset.id),
  )
  ...
  const { body: complexSearch2 } = await request(t.context.serverUrl)
    ...
    .send({
      filter:
        `_price >= ${basePrice} && _assetTypeId['typ_MnkfQps1I3a1gJYz2I3a']` +
  ...
  t.truthy(
    complexSearch2.results.find((asset) => assetsIds.asset13 === asset.id),
  )
})
...
test('[DSL-only] doesn’t return assets that match select filter if requiring multiple matches', async (t) => {
  ...
  // Two makes
  t.falsy(
    selectFilterSearch.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  t.falsy(
    selectFilterSearch.results.find((asset) => assetsIds.asset7 === asset.id),
  )
  ...
  t.truthy(
    selectFilterSearch2.results.find((asset) => assetsIds.asset6 === asset.id),
  )
  t.truthy(
    selectFilterSearch2.results.find((asset) => assetsIds.asset7 === asset.id),
  )
})
...
test('[DSL-only] throws appropriate error when using bad filter format', async (t) => {
  ...
  const { body: invalidListMatchValueError } = await request(
    t.context.serverUrl,
  )
    .post('/search')
    .set(authorizationHeaders)
  ...
  const { body: invalidListMatchOperatorError } = await request(
    t.context.serverUrl,
  )
    .post('/search')
    .set(authorizationHeaders)
