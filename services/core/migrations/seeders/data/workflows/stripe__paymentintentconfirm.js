module.exports = {
  checkOnboardingStatus: {
    name: '[Sync] Check onboarding status',
    description:
      'Checks if all the necessary information filled and marks onboarding status',
    event: 'user__updated',
    context: ['saltana'],
    computed: {
...
module.exports = {
  getWorkflows: (locale = 'en') => ({
    getEmailCheckTokenAtRegistration: {
      ...
      description:
        'Create a token used to validate the email address of a new user',
      event: 'user__created',
      context: ['stelace'],
      computed: {
    ...
    getEmailCheckTokenFromRequest: {
      ...
      description:
        'Create a token to validate a new email address filled by user',
      ...
      run: [
        {
          ...
          endpointPayload: {
            ...
            locale: `"${locale}"`,
            toEmail:
              'computed.isRegistration ? user.email : computed.tokenEmail',
    ...
    verifyEmail: {
      ...
      description:
        'The email check token is validated. Set the user email to verified.',
      ...
      run: [
        {
          ...
          endpointPayload: {
            email:
              'computed.replaceEmail && computed.email ? computed.email : undefined', // do not update email
            platformData: {
              _private: {
                newEmail:
                  'computed.replaceEmail && computed.email ? null : undefined', // do not update if no replace
              },
              instant: {
                emailVerified: true,
              },
    ...
    sendEmailPasswordReset: {
      ...
      computed: {
        ...
        toEmail: 'user.email',
        passwordResetLink:
          '`${env.STELACE_INSTANT_WEBSITE_URL}/?reset-password=${metadata.resetToken}`',
      },
      run: [
        {
          endpointMethod: 'POST',
      ...
      run: [
        ...
        },
        {
          ...
          computed: {
            ...
            conversationLink:
              '`${env.STELACE_INSTANT_WEBSITE_URL}/i/${responses.messages.results[0].conversationId}`',
          },
    ...
    captureStripePaymentIntent: {
      ...
      computed: {
        ...
        currencyDecimal:
          '_.get(transaction, "platformData.currencyDecimal", 2)',
      },
      run: [
        {
          endpointMethod: 'POST',
    ...
    onStripePaymentIntentConfirmation: {
      name: 'On Stripe payment intent confirmation',
      description: `
        When a payment intent is cancelled because the uncapture duration max limit is exceeded,
        the associated transaction must be cancelled too.
      ...
      run: [
        // find the user, if there is no under that email, create it
        {
          endpointMethod: 'GET',
          endpointPayload: {
        },
        // create a transaction for each asset
        {
          endpointMethod: 'POST',
          endpointUri: '/transactions',
        },
        // create an order with the transactions
        // shift order status to paid (or completed) with stripe payment intent id
        {
