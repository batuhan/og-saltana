...
function init(server, { middlewares, helpers } = {}) {
  ...
  server.get(
    ...
    checkPermissions(['user:list:all']),
  ...
  server.get(
    ...
    checkPermissions(['user:read', 'user:read:all']),
    wrapAction(async (req, res) => {
      const { id } = req.params
  ...
  server.patch(
    ...
    checkPermissions(['user:edit', 'user:edit:all'], {
      checkData: true,
      editProtectedNamespaces: true,
      optionalPermissions: ['user:config:all'],
    }),
  ...
  server.del(
    ...
    checkPermissions(['user:remove', 'user:remove:all']),
    wrapAction(async (req, res) => {
      const { id } = req.params
