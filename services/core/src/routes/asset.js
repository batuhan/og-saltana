...
function init(server, { middlewares, helpers } = {}) {
  ...
  server.get(
    ...
    checkPermissions(['asset:list', 'asset:list:all']),
  ...
  server.get(
    ...
    checkPermissions(['asset:read', 'asset:read:all']),
    wrapAction(async (req, res) => {
      const { id } = req.params
  ...
  server.post(
    ...
    checkPermissions(['asset:create', 'asset:create:all'], {
      checkData: true,
  ...
  server.patch(
    ...
    checkPermissions(['asset:edit', 'asset:edit:all'], {
      checkData: true,
      editProtectedNamespaces: true,
    }),
    wrapAction(async (req, res) => {
  ...
  server.del(
    ...
    checkPermissions(['asset:remove', 'asset:remove:all']),
    wrapAction(async (req, res) => {
      const { id } = req.params
