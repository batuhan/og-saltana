...
function init(server, { middlewares, helpers } = {}) {
  ...
  server.get(
    ...
    checkPermissions(['apiKey:list:all'], {
      optionalPermissions: [
        // needed for reveal
        'apiKey:create:all',
        'apiKey:edit:all',
  ...
  server.get(
    ...
    checkPermissions(['apiKey:read', 'apiKey:read:all'], {
      optionalPermissions: [
        // needed for reveal
        'apiKey:create:all',
        'apiKey:edit:all',
  ...
  server.post(
    ...
    checkPermissions(['apiKey:create:all'], {
      optionalCheck: true,
      checkData: true,
    }),
    wrapAction(async (req, res) => {
  ...
  server.patch(
    ...
    checkPermissions(['apiKey:edit', 'apiKey:edit:all'], { checkData: true }),
    wrapAction(async (req, res) => {
      const { id } = req.params
  ...
  server.del(
    ...
    checkPermissions(['apiKey:remove', 'apiKey:remove:all']),
    wrapAction(async (req, res) => {
      const { id } = req.params
