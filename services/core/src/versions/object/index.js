...
async function applyObjectSingleChange({ target, version, params }) {
  ...
  const newParams = await objectTransformer.applySingleChange({
    target,
    version,
    params,
  })
...
async function applyObjectChanges({ target, fromVersion, toVersion, params }) {
  ...
  const newParams = await objectTransformer.applyChanges({
    target,
    fromVersion,
    toVersion,
    params,
...
function registerObjectChanges(configChanges) {
  objectTransformer.addChanges(transformConfigsIntoChanges(configChanges))
}
