...
function start({ communication, serverPort }) {
  ...
  responder.on('read', async (req) => {
    ...
    if (populateLogs) {
      const queryBuilder = WorkflowLog.query().where({
        workflowId: workflow.id,
      })
      const actionCountQueryBuilder = queryBuilder.clone().where((qb) => {
        return qb
          .where((qb2) => {
            return (
              qb2
      ...
      const startedACountQueryBuilder = queryBuilder
        .clone()
        .whereNotIn('type', ['notification', 'preRunError'])
      const notificationCountQueryBuilder = queryBuilder
        .clone()
        .whereIn('type', ['notification'])
  ...
  responder.on('create', async (req) => {
    const { platformId } = req
    const { env } = req
    const { Event, Workflow } = await getModels({ platformId, env })
  ...
  responder.on('update', async (req) => {
    ...
    if (platformData) {
      updateAttrs.platformData = Workflow.rawJsonbMerge(
        'platformData',
        platformData,
      )
    }
  ...
  responder.on('listLogs', async (req) => {
    ...
    paginationMeta.results = WorkflowLog.exposeAll(paginationMeta.results, {
      req,
    })
  ...
  eventSubscriber.on(
    ...
    async ({ event, platformId, env } = {}) => {
      ...
      const prepareWorkflowsTransaction = apm.startTransaction(
        'Prepare Workflows',
        'workflow',
      )
    ...
    */
      ...
      try {
        const { Event, Workflow, WorkflowLog } = await getModels({
          platformId,
          env,
        })
        ...
        const workflows = await Workflow.query().where({
          active: true,
          event: event.type,
        })

        const workflowApiVersions = workflows.map(
          (workflow) => workflow.apiVersion || latestVersion,
        )

        if (workflows.length) {
          workflowsCtx = await bluebird.reduce(
            workflowApiVersions,
            async (ctx, apiVersion) => {
              const versionedEvent = await Event.getVersionedEvent(
                exposedEvent,
                apiVersion,
              )
              ctx[apiVersion] = { ...versionedEvent }

              // current custom events have no object type
              if (versionedEvent.objectType) {
                ctx[apiVersion][versionedEvent.objectType] =
                  ctx[apiVersion].object
              }
        ...
        if (workflows.length && !_.isEmpty(event.relatedObjectsIds)) {
          const { objectsTypes, objectsPromises: relatedObjectsPromises } =
          ...
          await bluebird.map(workflowApiVersions, async (apiVersion) => {
            const versionedObjects = await bluebird.reduce(
              Object.keys(objectsTypes),
              async (versioned, type) => {
        ...
        const workflowsHaveEnvVariables = workflows.some(
          (w) => !_.isEmpty(w.context),
        )
        if (workflowsHaveEnvVariables) {
          ...
          if (systemConfig.workflow) {
            ...
            workflows.forEach((w) => {
              ...
              envVariables[w.id] = w.context.reduce(
                (env, c) => Object.assign(env, contexts[c]),
                {},
              )
            })
          }
        ...
        await bluebird.mapSeries(workflows, async (workflow) => {
          ...
          const singleWorkflowTransaction = apm.startTransaction(
            'Execute Workflow',
            'workflow',
          )
          ...
          const initialComputedScript = _getComputedValuesScript(
            workflow.computed,
            { reset: true },
          )
          ...
          try {
            ...
            await bluebird
              .reduce(
                workflow.run,
                async (previousStepLog, workflowStep, i) => {
                  const runApmSpan = apm.startSpan(`Run step ${i}`)

                  const handledErr = i > 0 && workflow.run[i - 1].handleErrors

                  const isPreviousStepStopped =
                    previousStepLog.type === 'stopped'
                  const hasPreviousStepError = [
                    'preRunError',
                    'runError',
                  ].includes(previousStepLog.type)

                  if (isPreviousStepStopped) return previousStepLog
                  if (hasPreviousStepError && !handledErr)
                    return previousStepLog

                  // update computed object in each step
                  const currentStepComputedScript = _getComputedValuesScript(
                    workflowStep.computed,
                  )
                  const computedScript = `${
                    i === 0 ? initialComputedScript : ''
                  };${currentStepComputedScript}`

                  const prepareWorkflowSpan = apm.startSpan(
                  ...
                  try {
                    ...
                    if (passFilter && !skipStep) {
                      ...
                      debug(
                        `endpointHeaders: ${JSON.stringify(
                          endpointHeaders,
                          null,
                          2,
                        )}\n`,
                    ...
                    return WorkflowLog.query().insert({
                      ...
                      metadata: _getWorkflowLogMetadata({
                        workflowStep,
                        endpointUri,
                        endpointHeaders,
                        event,
      ...
      async function fetchRelatedObjects({
        ...
        for (const objectId in relatedObjectsObject) {
          ...
          const modelType = ((type) => {
            ...
            if (/^category/.test(type)) return 'category'
            if (/^asset/.test(type)) return 'asset'
            if (/^owner|^taker|^user/.test(type)) return 'user'
            if (/^transaction/.test(type)) return 'transaction'
          })(objectType)
          ...
          if (Model) {
            ...
            if (isValidRelatedObjectId) {
              objectsPromises[objectType] = Model.query().findById(id).execute()
            } else {
              objectsPromises[objectType] = Promise.resolve(null)
            }
          } else {
            const error = new Error(
              `Unknown event related object ${objectId} that can’t be mapped to a Model`,
            )
      ...
      function prepareWorkflowStep({
        ...
        const payloadScript = _populateScriptObjectValues(
          workflowStep.endpointPayload,
        )

        const headersScript = _.reduce(
          workflowStep.endpointHeaders || {},
          (script, v, k) => {
        ...
        vm.run(
          `responses = ${JSON.stringify(
            responses,
          )};ctx[apiVersion].responses = responses`,
        )
        ...
        if (envVariables[workflow.id]) {
          const envVariablesScript = _getEnvironmentVariablesScript(
            envVariables[workflow.id],
          )
          try {
        ...
        vm.run(
          `${ctxKeys.map((k) => `var ${k} = ctx[apiVersion].${k};`).join('')}`,
        )
        ...
        try {
          ...
          passFilter =
            !stop &&
            (!workflowStep.filter || Boolean(vm.run(workflowStep.filter)))
        ...
        debug(
          `vm ctx: ${JSON.stringify(vm.run('ctx[apiVersion]'), null, 2)}\n\n`,
        )
        ...
        function errorOrResult() {
          ...
          if (error) {
            ...
            prepareWorkflowError = _updatedLogWithErrorDetails({
              log,
              err: error,
            })
          }
      ...
      async function executeWorkflowStep({
        ...
        const headers = { ...endpointHeaders, ...saltanaHeaders }
        ...
        const endpointUrl = isInternalApiEndpoint
          ? `${apiBase}${endpointUri}`
          : endpointUri
        let isError

        return (
          request[method](endpointUrl)
...
function _transformVmError({ err, when, script }) {
  ...
  if (err.statusCode === 422) {
    ...
    newError = createError(422, `Script execution timed out ${when}`, {
      script,
    })
  } else if (err.name.includes('Error')) {
    newError = createError(
      err.statusCode || 422,
      `${err.name} ${when}: ${err.message}`,
      { script },
    )
  } else {
...
function _getComputedValuesScript(computedObject, { reset } = {}) {
  const script = reset
    ? 'computed = {}; ctx[apiVersion].computed = computed;'
    : ''

  return _populateScriptObjectValues(computedObject, '', {
    objectName: 'computed',
    script,
  })
}
 ...
 */
function _populateScriptObjectValues(
  object,
  path = '',
  { objectName = 'body', script = '' } = {},
) {
    ...
    if (Array.isArray(value)) {
      ...
      value.forEach((val, i) => {
        ...
        if (Array.isArray(val)) {
          ...
          script = _populateScriptObjectValues(val, accessor, {
            objectName,
            script,
          })
        } else if (_.isObjectLike(val)) {
          // we know it’s not an array
          script += `${objectName}${accessor} = {};`
          script = _populateScriptObjectValues(val, accessor, {
            objectName,
            script,
      ...
      // we know it’s not an array
      script += `${objectName}${fullPath} = {};`
      script = _populateScriptObjectValues(value, fullPath, {
        objectName,
        script,
      })
    } else {
...
function _getEnvironmentVariablesScript(envVariables) {
  return _.reduce(
    ...
    (script, v, k) => {
      ...
      return `${script} env["${k.replace(/"/g, '\\"')}"] = "${v.replace(
        /"/g,
        '\\"',
      )}";`
    },
