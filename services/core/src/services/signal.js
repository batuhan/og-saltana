...
function start({ communication, saltanaIO }) {
  ...
  saltanaIO.of('/signal').on('connection', function (socket) {
    ...
    async function authenticationCallback(data) {
      ...
      const channelsRequiringAuth = channels.filter((id) => {
        ...
        if (isUserChannel) {
          ...
          isOrgChannel = isValidObjectId({
            id,
            prefix: User.organizationIdPrefix,
            platformId,
            env,
          })
      ...
      if (userIds.length > 1)
        return disconnect('Can only subscribe to own user channel')
      ...
      if (needsAuthToken) {
        ...
        if (typeof authToken !== 'string')
          return disconnect('String authentication token expected')
        ...
        try {
          ...
          if (!isTokenExpired) {
            ...
            if (hasInternalUserId)
              channelsToJoin.push(prefixChannel(userId, { platformId, env }))
          }
        } catch (err) {
        ...
        await Promise.all(
          organizationIds.map(async (orgId) => {
            ...
            if (!isOrgMember)
              socket.emit(
                'warning',
                `${userId} was not granted access to ${orgId}.`,
              )
      ...
      if (nbClients >= maxNbClientsPerPlatform) {
      ...
      otherChannels = channels.filter(
        (c) => ![...organizationIds, ...userIds].includes(c),
      )
      if (otherChannels.length) {
        channelsToJoin.push(
          ...otherChannels.map((c) => prefixChannel(c, { platformId, env })),
        )
      }

      debug(
        `WebSocket ${socket.client.id} subscribing to ${channelsToJoin.join(
          ' ',
        )}`,
      )
    ...
    setTimeout(function () {
      ...
      socket.emit('willDisconnect', {
        error:
          disconnectReason ||
          `Authentication timeout after ${authenticationDelay}ms`,
      })
      socket.disconnect(true)

      debug(
        `WebSocket ${socket.client.id} disconnected due to ${
  ...
  responder.on('saltanaSignal', async (req) => {
    ...
    const { message, destination, event = 'signal' } = req

    if (!platformId || !env) throw createError(401)

    // Can’t broadcast to all users without appropriate permission
    if (!destination && !req._matchedPermissions['signal:create:all'])
    ...
    return new Promise((resolve, reject) => {
      saltanaIO.of('/signal').adapter.allRooms((err, rooms) => {
        ...
        if (destination && (!rooms || !rooms.includes(channel))) {
          return reject(
            createError(404, `Signal destination '${destination}' not found`),
          )
        }
