...
function start ({ communication, serverPort, isSystem }) {
  ...
  userRequester = getRequester({
    name: 'Authentication service > User Requester',
    key: 'user',
  })
  ...
  responder.on('logout', async (req) => {
    ...
    const { refreshToken: rawRefreshToken, logoutFromExternalProvider } = req
    ...
    if (!logoutFromExternalProvider) {
      ...
      let ssoConnection = await getSSOConnection({
        publicPlatformId,
        provider,
      })
      ...
      const logoutUrl = client.endSessionUrl({
        ...
        post_logout_redirect_uri: getSSOLogoutCallbackUrl({
          publicPlatformId,
          provider,
          serverPort,
        }),
  ...
  responder.on('checkToken', async (req) => {
    ...
    const { tokenType, token, userAgent } = req
  ...
  responder.on('ssoLogin', async (req) => {
    ...
    if (protocol === 'oauth2') {
      ...
      const authorizationParams = {
        redirect_uri: getSSOLoginCallbackUrl({
          publicPlatformId,
          provider,
          serverPort,
        }),
      ...
      const ssoLoginUrl = getSSOLoginUrlClient({
        client,
        authorizationParams,
        ssoConnection,
      })
    ...
    try {
      if (['oauth2', 'openid'].includes(protocol)) {
        ...
        if (protocol === 'oauth2') {
          ...
          const callbackURL = getSSOLoginCallbackUrl({
            publicPlatformId,
            provider,
            serverPort,
          })
        ...
        const { AuthMean, AuthToken, User } = await getModels({
          platformId,
          env,
        })
        ...
        if (shouldCreateUser) {
          ...
          await transaction(knex, async (trx) => {
            ...
            if (!Array.isArray(ssoProviders)) {
              // can happen if API consumers set this property to an arbitrary value
      ...
      if (afterAuthenticationUrl) {
        const params = {
          status:
            _.get(err, 'statusCode', 0) >= 500 ? 'serverError' : 'configError',
        }
  ...
  responder.on('ssoLogoutCallback', async (req) => {
    const { publicPlatformId, provider, state } = req
    ...
    const { protocol, afterAuthenticationUrl, afterLogoutUrl } = ssoConnection
  ...
  responder.on('changePassword', async (req) => {
    ...
    authMean = await AuthMean.query().patchAndFetchById(authMean.id, {
  ...
  responder.on('requestPasswordReset', async (req) => {
    const platformId = req.platformId
    const env = req.env
    const { AuthMean, AuthToken, Event, User } = await getModels({
      platformId,
      env,
    })
  ...
  responder.on('confirmPasswordReset', async (req) => {
    ...
    const { resetToken, newPassword } = req
    ...
    if (
      !authToken ||
      authToken.type !== 'resetPassword' ||
      authToken.value !== resetToken
    ) {
      throw createError(403)
    }
    ...
    if (!user) {
      throw createError(500, 'User associated with the reset token not found', {
        expose: false,
      })
    }
  ...
  responder.on('tokenRequestCheck', async (req) => {
    ...
    let { userId } = req
    const { tokenType: type, expirationDate, redirectUrl, data } = req
    ...
    let tokenValue = await getObjectId({
      prefix: tokenPrefix,
      separator: '',
      platformId,
      env,
  ...
  responder.on('tokenConfirmCheck', async (req) => {
    ...
    const updatedAuthToken = await AuthToken.query().patchAndFetchById(
      authToken.id,
      {
  ...
  responder.on('authCheck', async (req) => {
    const { apiKey, authorization } = req
    ...
    if (apiKey) {
      ...
      // `apiKey` and/or `token` whatever the scheme is (Basic or SaltanaCore-V1)
      // e.g. SaltanaCore-V1 apiKey=secretApiKey, token=secretToken
  ...
  async function impersonateToken ({
    ...
    if (isSystem(req._systemHash)) {
      ...
      const hasImpersonatePermissions =
        !!req._matchedPermissions['auth:impersonate']
      const isTargetUserOrganization =
        targetUser.roles.includes(organizationRole)
  ...
  async function createLoginTokensFromCode ({
    ...
    const result = createLoginTokens({
      platformId,
      env,
      user,
      userAgent,
    ...
    await AuthToken.query().patchAndFetchById(authToken.id, {
      ...
      if (fallbackRedirectUrl) {
        ...
        return {
          _rawResponse: {
            statusCode: 422,
            content:
              'This validation address is invalid, we invite you to contact staff.',
            headers: {
...
async function getOAuthClient ({
  ...
  if (protocol === 'openid') {
    ...
    if (jwks || jwksUrl) {
      issuerParams.jwks_uri =
        jwksUrl || getJwksUrl({ publicPlatformId, provider, serverPort })
    }
    if (idTokenSignedResponseAlg) {
      // node-openid-client default: 'RS256'
...
function getSSOLoginUrlClient ({ client, authorizationParams, ssoConnection }) {
  ...
  if (
    ...
    !_.isEmpty(ssoConnection.authorizationQueryParams)
  ) {
...
async function fetchOAuth2UserInfo ({ tokenSet, provider, client }) {
  ...
  const userInfo = await bluebird.reduce(
