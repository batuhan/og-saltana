...
function start({ communication }) {
  ...
  responder.on('create', async (req) => {
    ...
    if (
      !req._matchedPermissions['asset:create:all'] &&
      createAttrs.ownerId &&
    ...
    if (assetTypeId) {
      ...
      if (!assetTypes.length) {
        ...
        throw createError(
          400,
          'Missing asset type ID and not asset type with `isDefault` enabled',
        )
      }
    ...
    if (customAttributesObj) {
      ...
      const checkCustomAttribute = CustomAttribute.checkObject(
        customAttributesObj,
        customAttributes,
      )
      if (!checkCustomAttribute.result) {
        const errorFields = checkCustomAttribute.errors.map(
          (error) => error.name,
        )

        throw createError(
          422,
          `Invalid custom attributes (${errorFields.join(', ')})`,
          {
            public: {
  ...
  responder.on('update', async (req) => {
    ...
    const assetType = await AssetType.query().findById(
    ...
    */
    ...
    if (customAttributesObj) {
      updateAttrs.customAttributes = Asset.rawJsonbMerge(
        'customAttributes',
        customAttributesObj,
      )
    }
    ...
    if (platformData) {
      updateAttrs.platformData = Asset.rawJsonbMerge(
        'platformData',
        platformData,
      )
    }
    ...
    try {
      const shouldSync =
        typeof updateAttrs.assetTypeId !== 'undefined' ||
  ...
  responder.on('remove', async (req) => {
    ...
    const [{ count: nbTransactions }] = await Transaction.query()
      .count()
  ...
  subscriber.on(
    'assetCreated',
    async ({ asset, eventDate, platformId, env, req } = {}) => {
      try {
    ...
    async ({
      ...
      try {
        ...
        if (typeof deltas === 'undefined') {
          ...
          deltas = Event.getUpdatedEventDeltas(
            config,
            updateAttrsBeforeFullDataMerge || updateAttrs,
            asset,
          )
        }
        ...
        const exposedNewAsset = newAsset
          ? Asset.expose(newAsset, { req, namespaces: ['*'] })
          : {}
            ...
            return Event.createEvent(
              {
                ...
                changesRequested: Asset.expose(delta, {
                  req,
                  namespaces: ['*'],
                }),
          ...
          await Event.createEvent(
            {
              ...
              changesRequested: Asset.expose(changes, {
                req,
                namespaces: ['*'],
              }),
              metadata: eventMetadata,
  ...
  subscriber.on(
    'assetDeleted',
    async ({ assetId, asset, eventDate, platformId, env, req } = {}) => {
      try {
        const { Event, Asset } = await getModels({ platformId, env })
  ...
  responder.on('_changeQuantity', async (req) => {
    const { transaction, actionType, platformId, env } = req
    ...
    const newAsset = await Asset.query().patchAndFetchById(
      asset.id,
      updateAttrs,
    )
