...
function start({ communication }) {
  ...
  responder.on('list', async (req) => {
    ...
    paginationMeta.results = CustomAttribute.exposeAll(paginationMeta.results, {
      req,
    })
  ...
  responder.on('read', async (req) => {
    ...
    const { customAttributeId } = req

    const customAttribute = await CustomAttribute.query().findById(
      customAttributeId,
    )
    if (!customAttribute) {
  ...
  responder.on('create', async (req) => {
    ...
    if (isESReindexing) {
      throw createError(
        422,
        'Cannot create a Custom Attribute while reindexing is processing',
      )
    }
    ...
    if (customAttributes.length) {
      throw createError(
        422,
        `${name} Custom Attribute name has already been used.`,
      )
    }
    ...
    const customAttribute = await CustomAttribute.query().insert({
      id: await getObjectId({
        prefix: CustomAttribute.idPrefix,
        platformId,
        env,
      }),
      name,
    ...
    if (needReindex) {
      ...
      try {
        ...
        await updateMapping({
          ...
          env,
          type: 'asset',
          customAttributes: customAttributes.concat([customAttribute]),
        })
      } catch (err) {
        ...
        const exposedMessage =
          'This Custom Attribute cannot be created due to an indexing problem. ' +
  ...
  responder.on('update', async (req) => {
    ...
    const fields = ['listValues', 'metadata', 'platformData']

    const payload = _.pick(req, fields)

    const { listValues, metadata, platformData } = payload
    ...
    if (
      listValues &&
      !CustomAttribute.listValuesTypes.includes(customAttribute.type)
    ) {
      throw createError(
        422,
    ...
    if (!_.isEmpty(listValues)) {
      ...
      const [{ count: nbAssets }] = await Asset.query()
        .count()
        .whereJsonNotSubsetOf(key, listValues)
      if (nbAssets) {
        throw createError(
          422,
    ...
    if (platformData) {
      updateAttrs.platformData = CustomAttribute.rawJsonbMerge(
        'platformData',
        platformData,
      )
    }
  ...
  responder.on('remove', async (req) => {
    ...
    const referencingAssetsQuery = Asset.query().whereJsonHasAny(
      'customAttributes',
    ...
    */
    const referencingAssetIds = await referencingAssetsQuery
      .patch({
        customAttributes: raw(`"customAttributes" - '${customAttribute.name}'`),
      })
      .returning('id')
      ...
      .map(
        ...
        async (emptyAsset) => {
          ...
          try {
            ...
            const updateAttrs = {
              customAttributes: { [customAttribute.name]: null },
            }
            const eventDelta = {
            ...
            const eventMetadata = {
              saltanaComment: `customAttribute '${customAttribute.name}' automatically removed from asset before DELETE.`,
            }
  ...
  subscriber.on(
    'customAttributeCreated',
    async ({ customAttribute, eventDate, platformId, env } = {}) => {
      try {
        ...
        await Event.createEvent(
          {
            ...
            object: CustomAttribute.expose(customAttribute, {
              namespaces: ['*'],
            }),
          },
          { platformId, env },
        ...
        await Event.createEvent(
          {
            ...
            object: CustomAttribute.expose(newCustomAttribute, {
              namespaces: ['*'],
            }),
            changesRequested: CustomAttribute.expose(updateAttrs, {
              namespaces: ['*'],
            }),
          },
          { platformId, env },
