...
class Event extends Base {
  ...
  static get jsonSchema() {
    return {
      ...
      properties: {
        ...
        changesRequested: {
          // cf. hasChangesRequestedRegex
          type: ['object', 'null'],
          default: null,
        },
        relatedObjectsIds: {
          // cf. hasRelatedObjectsIdsRegex
        ...
        parentId: {
          // for events derived from updated events such as asset__name_changed
          type: ['string', 'null'],
          default: null,
        },
        emitter: {
          // 'core' or 'custom' or 'task'
          type: 'string',
          maxLength: 255,
        },
        emitterId: {
  ...
  $beforeInsert() {
    ...
    if (this.emitter === 'core') {
      if (!this.objectType && this.type)
        this.objectType = _.camelCase(this.type.split('__')[0])

      const parsedObjectId =
        _.get(this.object || this._tmpObject, 'id') || this.objectId || null // null matters here, see below

      const initialObjectId = this.objectId
      this.objectId = this.objectId || parsedObjectId

      if (this.objectId !== parsedObjectId) {
        // Note that undefined !== null
        // meaning given objectId shall not be undefined if it can’t be parsed in (_tmp)object
        throw new Error(
          `Invalid objectId ${initialObjectId} in ${this.type} event`,
          {
            parsedObjectId,
          },
        )
  ...
  static async createEvent(params, { platformId, env, queryContext } = {}) {
    ...
    if (
      newParams.changesRequested &&
      typeof newParams.changesRequested.livemode !== 'undefined'
    ) {
  ...
  static getBadCustomEventTypeMessage() {
    return (
      'Cannot provide a type with 2 underscore characters' +
  ...
  static isAllowedEvent(event) {
    ...
    return (
      (isCoreFormat && this.isAllowedCoreEvents(event)) ||
  ...
  static getUpdatedEventDeltasConfig(objectType) {
    if (typeof objectType !== 'string') {
      throw new Error(
        `string objectType expected in deltas config, got ${typeof objectType}`,
      )
    }
    ...
    const deltasConfigs = {
      asset: {
        ...
        asset__pricing_changed: ['price', 'currency'],
        ...
        asset__custom_attribute_changed: (updateAttrs, asset) => {
          ...
          if (_.isEqual(updateAttrs.customAttributes, asset.customAttributes))
            return {}
    ...
    if (!Object.keys(deltasConfigs).includes(objectType)) {
      throw new Error(
        `Missing internal deltas config when emitting ${objectType}__updated event`,
      )
    }
  ...
  static async getVersionedEvent(event, toVersion) {
    ...
    const [changesRequested, object] = await Promise.all([
      applyObjectChanges({
        fromVersion,
        toVersion,
        target: objectType,
