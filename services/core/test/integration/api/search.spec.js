...
test('synchronizes assets when there are any changes', async (t) => {
  ...
  t.truthy(
    searchResult.results.find(
      (asset) => asset.name === 'Ford' && assetsIds.asset3 === asset.id,
    ),
  )
  t.truthy(
    searchResult.results.find(
      (asset) => asset.name === 'Custom Ford' && assetsIds.asset2 === asset.id,
    ),
  )
...
test('returns assets filtered by created date', async (t) => {
  ...
  const {
    body: { results: resultsWithoutCreatedFilter },
  ...
  const {
    body: { results: resultsWithCreatedBeforeFilter },
  } = await request(t.context.serverUrl)
    .post('/search')
    .set(authorizationHeaders)
    .send({
  ...
  const {
    body: { results: resultsWithCreatedAfterFilter },
  } = await request(t.context.serverUrl)
    .post('/search')
    .set(authorizationHeaders)
    .send({
      ...
      location: { latitude: 2, longitude: 2 },
      maxDistance: 10000,
      createdAfter: computeDate(
        resultsWithoutCreatedFilter[0].createdDate,
        '1ms',
      ),
    })
    .expect(200)
  ...
  t.falsy(
    resultsWithCreatedAfterFilter.find(
      (asset) => resultsWithoutCreatedFilter[0].id === asset.id,
    ),
  )
})
...
test('returns similar assets', async (t) => {
  const authorizationHeaders = await getAccessTokenHeaders({
    t,
    permissions: ['asset:read:all', 'search:list:all'],
  })
...
test('transactions block availability based on asset type unavailableWhen parameter', async (t) => {
  ...
  t.true(
    startSearchResult1.results[0].id === scenario1SearchResult1.results[0].id,
  )
  ...
  t.true(
    startSearchResult2.results[0].id === scenario1SearchResult2.results[0].id,
  )
  ...
  t.true(
    startSearchResult1.results[0].id === scenario2SearchResult1.results[0].id,
  )
  ...
  t.true(
    startSearchResult2.results[0].id === scenario2SearchResult2.results[0].id,
  )
  ...
  t.true(
    startSearchResult1.results[0].id === scenario3SearchResult1.results[0].id,
  )
  ...
  t.true(
    startSearchResult2.results[0].id === scenario3SearchResult2.results[0].id,
  )
})
...
test('sorts assets by price', async (t) => {
  ...
  t.true(
    priceSort.results.reduce((higherPrice, asset, i, assets) => {
  ...
  t.true(
    priceSort2.results.reduce((lowerPrice, asset, i, assets) => {
...
test('sorts assets by availability and price', async (t) => {
  ...
  const {
    ...
    .set(authorizationHeaders)
    .send({
      sort: [{ _available: 'asc' }, { _price: 'desc' }],
      availabilityFilter: {
        enabled: false,
      },
      startDate: computeDate(initNow, '11 days'),
  ...
  let availabilities = []
  const isOrderedByAscendingAvailability = searchResults.reduce(
    (memo, asset, index) => {
      const assetAvailable = asset.available
      const previousAssetAvailable =
        searchResults[Math.max(0, index - 1)].available
  ...
  availabilities.forEach((available) => {
    ...
    isThenOrderedByDescendingPrice =
      isThenOrderedByDescendingPrice &&
      searchResults.reduce((memo, asset) => {
        ...
        const hasLowerPrice =
          asset.price <= (previousAssetWithSameAvailability || asset).price
  ...
  const {
    ...
    .set(authorizationHeaders)
    .send({
      sort: [{ _available: 'desc' }, { _price: 'asc' }],
      availabilityFilter: {
        enabled: false,
      },
      startDate: computeDate(initNow, '11 days'),
  ...
  availabilities = []
  const isOrderedByDescendingAvailability = searchResults2.reduce(
    (memo, asset, index) => {
      const assetAvailable = asset.available
      const previousAssetAvailable =
        searchResults2[Math.max(0, index - 1)].available
...
test('sorts assets by price and custom attributes of type number', async (t) => {
  ...
  const {
    ...
    .set(authorizationHeaders)
    .send({
      sort: [{ seatingCapacity: 'asc' }, { _price: 'desc' }],
    })
    .expect(200)

  let seatingCapacities = []
  const isOrderedByAscendingSeatingCapacity = searchResults.reduce(
    (memo, asset, index) => {
  ...
  seatingCapacities.forEach((capacity) => {
    ...
    isThenOrderedByDescendingPrice =
      ...
      searchResults.reduce((memo, asset) => {
        ...
        const hasLowerPrice =
          asset.price <= (previousAssetWithSameCapacity || asset).price
  ...
  const {
    ...
    .set(authorizationHeaders)
    .send({
      sort: [{ seatingCapacity: 'desc' }, { _price: 'asc' }],
    })
    .expect(200)

  seatingCapacities = []
  const isOrderedByDescendingSeatingCapacity = searchResults2.reduce(
    (memo, asset, index) => {
...
test('sorts assets by several custom attributes of type number, including null values', async (t) => {
  ...
  const {
    ...
    .set(authorizationHeaders)
    .send({
      sort: [{ seatingCapacity: 'desc' }, { customScore: 'asc' }],
    })
    .expect(200)

  const seatingCapacities = []
  const isOrderedByDescendingSeatingCapacity = searchResults.reduce(
    (memo, asset, index) => {
...
test('sorts assets by custom attribute of type text, with missing values last', async (t) => {
  ...
  const isSortedByAscendingText = searchResults.reduce((memo, asset, index) => {
    const lastPlate =
      searchResults[Math.max(0, index - 1)].customAttributes.licensePlate
    // Missing/null values are sorted "_last" in ElasticSearch (~ highestTextValue)
    const lastPlateWithFallback = nullOrUndefinedFallback(
      lastPlate,
      highestTextValue,
    )
  ...
  const isSortedByDescendingText = searchResults2.reduce(
    (memo, asset, index) => {
      const lastPlate =
        searchResults2[Math.max(0, index - 1)].customAttributes.licensePlate
...
test('sorts assets by custom attribute of type boolean, with missing values last', async (t) => {
  ...
  const isSortedByAscendingBoolean = searchResults.reduce(
    (memo, asset, index) => {
      const lastBoolean =
        searchResults[Math.max(0, index - 1)].customAttributes
...
test('sorts assets by created date', async (t) => {
  ...
  const isOrderedByDescendingCreatedDate = searchResults.reduce(
    (memo, asset, index) => {
      const previousIndex = Math.max(0, index - 1)
...
test('sorts assets by name', async (t) => {
  ...
  const isOrderedByDescendingName = searchResults.reduce(
    (memo, asset, index) => {
      const previousIndex = Math.max(0, index - 1)
...
test('should retrieve available results even if they are beyond the first page of Elasticsearch', async (t) => {
  const authorizationHeaders = await getAccessTokenHeaders({
    t,
    permissions: ['asset:create:all', 'search:list:all'],
  })
...
test('should work with all languages', async (t) => {
  ...
  t.true(searchResult1.results.length > 0)
  t.truthy(
    searchResult1.results.find((asset) => assetsIds.asset11 === asset.id),
  )
  ...
  t.true(searchResult2.results.length > 0)
  t.truthy(
    searchResult2.results.find((asset) => assetsIds.asset11 === asset.id),
  )
  ...
  t.truthy(
    searchResult3.results.find((asset) =>
      [assetsIds.asset13, assetsIds.asset14].includes(asset.id),
    ),
  )
  ...
  t.truthy(
    searchResult4.results.find((asset) => assetsIds.asset13 === asset.id),
  )
})
...
test.serial('generates asset__searched event', async (t) => {
  const authorizationHeaders = await getAccessTokenHeaders({
    t,
    permissions: ['search:list:all', 'event:list:all'],
  })
