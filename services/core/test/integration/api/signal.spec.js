...
test.before(async (t) => {
  ...
  const createSocket = (name, cbData, waitMs = 0) => {
    sockets[name] = io.connect(
      `http://localhost:${t.context.serverPort}/signal`,
      { path: '/signal' },
    )
    ...
    return new Promise((resolve) => {
      sockets[name].on(
        'authentication',
        async function (socketId, subscribeCb) {
          sockets[name].on('signal', (data) => socketsData[name].push(data))
          sockets[name].on(eventName, (data) => socketsData[name].push(data))
          sockets[name].on('warning', (w) =>
            socketsData[name].push(`warning: ${w}`),
          )
          sockets[name].on('willDisconnect', (r) =>
            socketsData[name].push(`willDisconnect: ${r.error}`),
          )
          // native event
          sockets[name].on('disconnect', (reason) =>
            socketsData[name].push(`disconnect: ${reason}`),
          )
  ...
  await createSocket('notSomeOrgMember', {
    ...
    channels: [
      `${userId}_friends_channel`,
      userOrganizationId,
      notUserOrganizationId,
    ],
    authToken: userAccessToken, // automatically joining userId channel
    publishableKey: apiKey.key,
  })
...
test('emits signal to single client', async (t) => {
  ...
  t.false(
    socketsData.missingAuthTokenForOrg.some((d) => d.message === message1),
  )
  t.false(
    socketsData.missingAuthTokenForUserIdChannel.some(
      (d) => d.message === message1,
    ),
  )
...
test('broadcasts signal to all authenticated clients with appropriate permissions', async (t) => {
  ...
  t.true(
    socketsData.authenticatedOrg.some((d) => _.isEqual(d.message, message2)),
  )
  t.true(
    socketsData.notSomeOrgMember.some((d) => _.isEqual(d.message, message2)),
  )

  // Unauthenticated socket
  t.false(socketsData.invalidApiKey.some((d) => _.isEqual(d.message, message2)))
  t.false(
    socketsData.missingAuthTokenForOrg.some((d) =>
      _.isEqual(d.message, message2),
    ),
  )
  t.false(
    socketsData.missingAuthTokenForUserIdChannel.some((d) =>
      _.isEqual(d.message, message2),
    ),
  )
...
test('broadcasts named signal event to all authenticated clients', async (t) => {
  ...
  t.true(
    socketsData.authenticatedOrg.some((d) => _.isEqual(d.event, eventName)),
  )
  t.true(
    socketsData.notSomeOrgMember.some((d) => _.isEqual(d.event, eventName)),
  )
...
test('emits warning when trying to join channel of an organization user is not a member of', async (t) => {
  t.true(
    socketsData.notSomeOrgMember.some(
      (d) =>
        typeof d === 'string' &&
        d.startsWith('warning:') &&
        d.includes('not granted'),
    ),
  )
})
...
test('disconnects unauthenticated or invalid clients', async (t) => {
  ...
  t.false(
    validSockets.some((sock) => {
      const disconnect = sock.some(
        (d) => typeof d === 'string' && d.startsWith('disconnect:'),
      )
      const willDisconnect = sock.some(
        (d) => typeof d === 'string' && d.startsWith('willDisconnect:'),
      )
  ...
  invalidSockets.forEach((sock, index) => {
    ...
    t.true(
      sock.some(
        (d) =>
          typeof d === 'string' &&
          d.startsWith('disconnect:') &&
          d.includes('server'),
      ),
    )
  })
  t.true(
    socketsData.invalidApiKey.some(
      (d) => typeof d === 'string' && d.includes('key'),
    ),
  )
  t.true(
    socketsData.missingAuthTokenForOrg.some(
      (d) => typeof d === 'string' && d.includes('token'),
    ),
  )
  t.true(
    socketsData.missingAuthTokenForUserIdChannel.some(
  ...
  t.true(
    socketsData.severalUserIds.some(
      (d) => typeof d === 'string' && d.includes('own'),
    ),
  )
  t.true(
    socketsData.timeout.some(
      (d) => typeof d === 'string' && d.includes('timeout'),
    ),
  )
})
