 ...
 */
async function findDeepKey(
  obj,
  key,
  { getPath = false, base = '', arrayResults = false } = {},
) {
  ...
  if (arrayResults && Array.isArray(tmpObj)) {
    const foundObjects = await Promise.all(
      obj.map(async (v, i) => {
        const opts = {
          base: _appendPath(tmpPath, i),
          getPath: true,
          arrayResults,
        }
...
async function allDeepKeys(object, key, options = {}) {
  ...
  if (
    [options.copyTo, options.moveTo].some((o) => o && typeof o !== 'string')
  ) {
  ...
  while ((found = await findDeepKey(copy, key, innerOptions)) && found.result) {
    // eslint-disable-line
    ...
    if (elapsed > maxDuration)
      throw new Error(`allDeepKeys taking more than ${maxDuration}.`, object)
  }
