...
const matchCursorResult = _.curry((decodedCursor, result) => {
  const normalizeValue = (prop) =>
    prop instanceof Date ? prop.toISOString() : prop
...
async function offsetPaginate({
  ...
  queryBuilder.offset((page - 1) * nbResultsPerPage).limit(nbResultsPerPage)

  const [results, [{ count: nbResults }]] = await Promise.all([
    queryBuilder,
...
async function cursorPaginate({
  ...
  if (startingAfter && endingBefore)
    throw new Error('Cannot provide both `startingAfter` and `endingBefore`')
  ...
  if (decodedCursor)
    results = results.filter(_.negate(matchCursorResult(decodedCursor)))
  ...
  paginationMeta.startCursor = firstResult
    ? createCursor(firstResult, cursorProps)
    : null
  paginationMeta.endCursor = lastResult
    ? createCursor(lastResult, cursorProps)
    : null
...
function getCursorPaginationMeta({
  ...
  } else if (endingBefore) hasPreviousPage = results.length > nbResultsPerPage
...
function applyCursorPaginationParameters({
  ...
  return queryBuilder.where((qb) => {
    ...
    if (cursorProps.length === 1) {
      ...
      qb.where(firstProp, operator, decodedCursor[firstProp]).orWhere((qb2) => {
        return qb2
