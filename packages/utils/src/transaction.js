...
const { getPriceAfterFees, roundPriceWithCurrency } = require('./pricing')
...
function getFutureStatuses(status, { excludeCurrentStatus = true } = {}) {
  ...
  function _addNextStatuses(status) {
    ...
    const statusTransitions = (indexedFromStatuses[status] || []).concat(
      indexedFromStatuses['*'] || [],
    )
...
function getTransactionProcess(transaction) {
  ...
  if (!assetType || !assetType.transactionProcess)
    return defaultTransactionProcess
...
function getBlockingAvailabilityChange(transaction) {
  return {
    previous:
      transaction.statusHistory.length < 2
        ? false
        : isStatusBlockingAvailability(
            transaction,
            transaction.statusHistory[1].status,
          ),
...
function isValidDates({
  ...
  if (!isDateString(startDate)) {
    errors.push({
      message:
        'Start date has an invalid format (expected format: YYYY-MM-DDTHH:MM:SS.sssZ)',
      value: startDate,
    })
  }
  if (!isDateString(refDate)) {
    errors.push({
      message:
        'Reference date has an invalid format (expected format: YYYY-MM-DDTHH:MM:SS.sssZ)',
      value: refDate,
    })
  }
  if (errors.length) {
...
function getTransactionDurationData({
  startDate,
  endDate,
  duration,
  timeUnit = 'd',
 ...
 */
function getTransactionPricing(
  transaction,
  { value, ownerAmount, takerAmount } = {},
) {
  const { ownerFeesPercent, takerFeesPercent } = transaction.assetType.pricing
  ...
  const defaultValue =
    transaction.unitPrice * nbTimeUnits * (transaction.quantity || 1)
  ...
  const priceResult = {
    value: finalValue,
    ownerAmount: _.isFinite(ownerAmount)
      ? ownerAmount
      : defaultPriceResult.ownerNetIncome,
    takerAmount: _.isFinite(takerAmount)
      ? takerAmount
      : defaultPriceResult.takerPrice,
  }

  priceResult.takerFees = roundPrice(
    priceResult.takerAmount - priceResult.value,
  )
  priceResult.ownerFees = roundPrice(
    priceResult.value - priceResult.ownerAmount,
  )
  priceResult.platformAmount = roundPrice(
    priceResult.ownerFees + priceResult.takerFees,
  )
