...
const { findDeepKey, allDeepKeys } = require('./deepKeys')
...
test('findDeepKey returns first object found in array or array of results using arrayResults option', async (t) => {
  const results = await findDeepKey(array, 'platformData', {
    arrayResults: true,
  })
...
test('allDeepKeys returns first object found in array', async (t) => {
  ...
  t.deepEqual(
    await allDeepKeys(array, 'platformData', { getPath: true }),
    array.map((o, i) => {
...
test('allDeepKeys returns several objects in nested array', async (t) => {
  ...
  t.true(
    result.every(
      (r) => r.result.other === _.get(severalInArray, `${r.path}.other`)
    )
  )
