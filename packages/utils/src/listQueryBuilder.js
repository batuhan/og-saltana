...
const rangeFilterSchema = Joi.alternatives()
  .try(
    Joi.any(),
    Joi.object().pattern(
...
const orderConfigSchema = Joi.object()
  .keys({
...
const offsetPaginationConfigSchema = Joi.object()
  .keys({
...
const cursorPaginationConfigSchema = Joi.object()
  .keys({
    nbResultsPerPage: Joi.number().integer().min(1).required(),
...
function getTransformedValues(filters) {
  ...
  Object.keys(filters).forEach((key) => {
    ...
    if (_.isUndefined(transformValue)) {
      ...
      throw new Error(
        `Unknown transformation value for filter on field ${filter.dbField}`,
      )
    }

    if (_.isUndefined(newValue) && !_.isUndefined(defaultValue))
    ...
    if (_.isUndefined(query)) {
      ...
      if (query === 'range') {
        ...
        const throwMinValueError = () => {
          throw createError(
            400,
            `${key} value cannot be lower than ${minValue}`,
          )
        }
        ...
        if (!_.isUndefined(minRangeValue)) {
          ...
          if (
            !isSingleValue &&
            _.isUndefined(transformedValue.gt) &&
            _.isUndefined(transformedValue.gte)
          ) {
        ...
        whereJsonbColumnSupersetOf(queryBuilder, {
          columnName: dbField,
          data: value,
        })
      }
...
function checkOffsetPaginationConfig(paginationConfig) {
  const { error } = offsetPaginationConfigSchema.validate(
    paginationConfig,
    joiOptions,
  )
...
function checkCursorPaginationConfig(paginationConfig) {
  const { error } = cursorPaginationConfigSchema.validate(
    paginationConfig,
    joiOptions,
  )
...
async function performListQuery({
  ...
  if (paginationActive) {
    if (useOffsetPagination) {
      checkOffsetPaginationConfig(
        _.pick(paginationConfig, ['page', 'nbResultsPerPage']),
      )
    } else {
      checkCursorPaginationConfig(
        _.pick(paginationConfig, [
          'startingAfter',
          'endingBefore',
          'nbResultsPerPage',
        ]),
...
async function performAggregationQuery({
  ...
  queryBuilder.select().from('aggregations').whereNotNull('groupByField')
  ...
  paginationMeta.results = paginationMeta.results.map((r) => {
    ...
    if (clonedResult.count)
      clonedResult.count = parseInt(clonedResult.count, 10)
    if (_.isNumber(clonedResult.avg))
      clonedResult.avg = roundDecimal(clonedResult.avg, avgPrecision)
...
async function performHistoryQuery({
  ...
  if (filters) {
    ...
    const timeRestrictedFilterKeys = _.without(
      Object.keys(filters),
      ..._.compact([timeFilter, secondaryFilter]),
    )
    useOnlyNonTimeRestrictedFilters = timeRestrictedFilterKeys
      .map((key) => _.get(filters, `${key}.value`))
      .every(_.isUndefined)
    const minRangeValue = getMinRangeValue(filters[timeFilter].value)

    if (
      !_.isUndefined(minRangeValue) &&
      minRangeValue < retentionLimitDate &&
      !useOnlyNonTimeRestrictedFilters
    ) {
      throw createError(
        400,
        `All filters are disabled before ${retentionLimitDate}`,
      )
    }
  }
  ...
  if (!useContinuousAggregate || (useContinuousAggregate && secondaryFilter))
  ...
  const cursorProps = [{ name: orderBy, type: 'date' }]
