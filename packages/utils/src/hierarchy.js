...
function isValidHierarchy(
  elements,
  { idField = 'id', parentIdField = 'parentId', idFn, parentIdFn } = {},
) {
  const checkedIds = {}
  let checkingIds = {}
  ...
  const checkElement = (element) => {
    ...
    if (typeof parentId === 'undefined' || parentId === null) {
      validateChain(id)
    } else if (checkingIds[parentId]) {
      // if there is a circular dependency (parent is in the working chain)
      valid = false
    } else if (id === parentId) {
      // the element references itself as parent
      valid = false
    } else if (checkedIds[parentId]) {
      // the parent chain has already been validated
      validateChain(id)
    } else {
