...
const { computeRecurringPeriods } = require('./time')
 ...
 */
function getAvailabilityPeriodGraph({
  transactions = [],
  availabilities = [],
  defaultQuantity,
  maxQuantity,
  ...
  const allAvailabilities =
    _getAvailabilitiesWithRecurringPattern(availabilities)

  const { fixedQuantityAvailabilities, relativeQuantityAvailabilities } =
    _partitionFixedAndRelativeQuantityAvailabilities(allAvailabilities)

  const { indexedFixedQuantityAvailabilities, fixedQuantityDates } =
    _getIndexedFixedQuantityAvailabilities(fixedQuantityAvailabilities)

  const indexedRelativeQuantityAvailabilitiesByStartDate = _.groupBy(
    relativeQuantityAvailabilities,
    'startDate',
  )
  const indexedRelativeQuantityAvailabilitiesByEndDate = _.groupBy(
    relativeQuantityAvailabilities,
    'endDate',
  )
  ...
  dates.forEach((date) => {
    const addUsedQuantity = (indexedTransactionsByStartDate[date] || []).reduce(
      (memo, transaction) => {
    ...
    const removeUsedQuantity = (
      indexedTransactionsByEndDate[date] || []
    ).reduce((memo, transaction) => {
      memo += transaction.quantity
      return memo
    }, 0)

    const addAvailableQuantity = (
      indexedRelativeQuantityAvailabilitiesByStartDate[date] || []
    ).reduce((memo, availability) => {
      memo += availability.quantity
      return memo
    }, 0)
    const removeAvailableQuantity = (
      indexedRelativeQuantityAvailabilitiesByEndDate[date] || []
    ).reduce((memo, availability) => {
      memo += availability.quantity
      return memo
    }, 0)
    ...
    if (typeof fixedAvailability !== 'undefined') {
      fixedQuantity = fixedAvailability
        ? fixedAvailability.quantity
        : defaultQuantity
    }
    ...
    relativeQuantity =
      relativeQuantity + addAvailableQuantity - removeAvailableQuantity
...
function _getAvailabilitiesWithRecurringPattern(availabilities) {
  ...
  availabilities.forEach((availability) => {
    const isRecurring =
      ...
      availability.recurringTimezone &&
      availability.recurringDuration

    if (isRecurring) {
      const recurringPeriods = computeRecurringPeriods(
        availability.recurringPattern,
        {
...
function _getIndexedFixedQuantityAvailabilities(fixedQuantityAvailabilities) {
  ...
  fixedQuantityAvailabilities.forEach((availability) => {
    if (!indexedAddFixedQuantityAvailabilities[availability.startDate]) {
      indexedAddFixedQuantityAvailabilities[availability.startDate] = [
        availability,
      ]
    } else {
      indexedAddFixedQuantityAvailabilities[availability.startDate].push(
        availability,
      )
    }
    if (!indexedRemoveFixedQuantityAvailabilities[availability.endDate]) {
      indexedRemoveFixedQuantityAvailabilities[availability.endDate] = [
        availability,
      ]
    } else {
      indexedRemoveFixedQuantityAvailabilities[availability.endDate].push(
        availability,
      )
    }
  ...
  fixedQuantityDates.forEach((date) => {
    ...
    poolFixedQuantityAvailabilities = _.differenceBy(
      poolFixedQuantityAvailabilities,
      removeAv,
      (av) => av.endDate,
    )
    poolFixedQuantityAvailabilities =
      poolFixedQuantityAvailabilities.concat(addAv)
    ...
    indexedFixedQuantityAvailabilities[date] =
      poolFixedQuantityAvailabilities[0] || null
  })
...
function getAvailabilityPeriodInfo(availabilityGraph, newTransaction) {
  ...
  const beforeStartGraphDate = _.last(
    graphDates.filter(
      (graphDate) => graphDate.date <= newTransaction.startDate,
    ),
  )

  const overlapGraphDates = graphDates.filter((graphDate) => {
    const startDate = beforeStartGraphDate
      ? beforeStartGraphDate.date
  ...
  if (!overlapGraphDates.length) {
    ...
    remainingQuantity = Math.abs(
      overlapGraphDates[0].availableQuantity -
        overlapGraphDates[0].usedQuantity,
    )

    overlapGraphDates.forEach((graphDate) => {
      remainingQuantity = Math.min(
        remainingQuantity,
        Math.abs(graphDate.availableQuantity - graphDate.usedQuantity),
      )
    })
  }
  ...
  return {
    isAvailable:
      newTransaction.quantity <= remainingQuantity && remainingQuantity > 0,
    remainingQuantity,
  }
}
...
function _breakIntoDateChunks(dateElement, dates) {
  ...
  innerDates.forEach((innerDate) => {
    const obj = { ...dateElement, startDate: prevDate, endDate: innerDate }
...
function getAvailabilityPeriods(availabilityGraph) {
  ...
  if (!graphDates.length) {
    ...
    graphDates.forEach((graphDate, index) => {
      const { date, availableQuantity } = graphDate
