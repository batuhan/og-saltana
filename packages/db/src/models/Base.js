...
const { Model, QueryBuilder } = require('@saltana/objection')
const { DbErrors } = require('objection-db-errors')

const _ = require('lodash')

const {
  merging: { mergeOrOverwrite },
  systemNamespaces,
  protectedNamespaces,
} = require('@saltana/utils')

function isSystem(hash, systemHash = process.env.SYSTEM_HASH || 'system') {
...
class Base extends DbErrors(Model) {
  ...
  static expose(
    ...
    if (locale && typeof this.getI18nMap === 'function') {
      object = this.getI18nModel(object, {
        locale,
        fallbackLocale,
      })
    }
    ...
    accessFields.forEach((field) => {
      this.exposeTransform(object, field, {
        access,
        namespaces,
        plan,
        planPermissions,
    ...
    if (!exposeAllNamespaces) {
      if (hasWildcardNamespace) {
        const indexedSystemNamespaces = systemNamespaces.reduce(
          (memo, namespace) => {
        ...
        if (
          exposedObject.metadata &&
          typeof exposedObject.metadata === 'object'
        ) {
        ...
        if (
          exposedObject.platformData &&
          typeof exposedObject.platformData === 'object'
        ) {
  ...
  static getI18nModelDelta(element, attrs, { locale, fallbackLocale }) {
    ...
    return getI18nModelDelta(element, attrs, {
      i18nMap,
      locale,
      fallbackLocale,
    })
  ...
  static getCustomData(element, params) {
    ...
    if (!hasOneDataObject) {
      throw new Error(
        'Only one of metadata, platformData, customAttributes or newData is expected.'
      )
    }
    if (newData && !field) {
      throw new Error(
        'Field name is required when using newData rather than data built-ins.'
      )
    }
...
function getI18nModel(
  model,
  { i18nMap = {}, locale, fallbackLocale, useOnlyLocale }
) {
...
function getI18nModelDelta(
  model,
  attrs,
  { i18nMap = {}, locale, fallbackLocale }
) {
  const delta = {}
...
function setI18nModel(model, attrs, { i18nMap = {}, locale, fallbackLocale }) {
  Object.keys(i18nMap).forEach((key) => {
    const value = attrs[key]
...
function getI18nValue(
  model,
  { field, fieldI18n, locale, fallbackLocale, useOnlyLocale = false }
) {
...
function setI18nValue(
  model,
  value,
  { field, fieldI18n, locale, fallbackLocale }
) {
  model[fieldI18n] = model[fieldI18n] || {}
  model[fieldI18n][locale] = value

  const existFallback =
    fallbackLocale && typeof model[fieldI18n][fallbackLocale] === 'string'
