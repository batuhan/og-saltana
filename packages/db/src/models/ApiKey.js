const { parseKey, getBaseKey } = require('@saltana/util-keys')
    ...
    switch (field) {
      case 'key': {
        ...
        if (key && typeof key === 'string') {
          ...
          const shouldObfuscateKey =
            !['pubk', 'cntk'].includes(parsedKey.type) && !revealKey
          ...
          if (shouldObfuscateKey) {
            ...
            element.key =
              key.slice(0, baseKey.length + nbKeyCharsToShow) +
              key
                .slice(baseKey.length + nbKeyCharsToShow, -nbKeyCharsToShow)
                .replace(/\w/gi, 'x') +
