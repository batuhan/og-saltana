...
class Order extends Base {
  ...
  static expose(element, options = {}) {
    const exposedElement = super.expose(element, {
      ...options,
      getAccessFieldsFn: this.getAccessFields,
    })

    if (exposedElement.lines) {
      exposedElement.lines = element.lines.map((line) =>
        this.exposeLine(line, options),
      )
    }
    if (exposedElement.moves) {
      exposedElement.moves = element.moves.map((move) =>
        this.exposeMove(move, options),
      )
    }
