...
const RawTransaction = require('./Transaction')
const RawInternalAvailability = require('./InternalAvailability')
const RawAsset = require('./Asset')
const RawAvailability = require('./Availability')
const RawAssetType = require('./AssetType')
const {
  availability: { getAvailabilityPeriodGraph, getInternalAvailabilityPeriods },
  transaction: { shouldAffectAvailability, isStatusBlockingAvailability },
...
class InternalAvailability extends Base {
  ...
  static async syncInternalAvailability({ assetsIds, platformId, env }) {
    const {
      ...
      models: {
        ...
        InternalAvailability: RawInternalAvailability,
        Asset: RawAsset,
    ...
    const [assets, assetTypes, allAssetAvailabilities, allTransactions] =
      await Promise.all([
    ...
    const indexedAssetAvailabilities = _.groupBy(
    ...
    await bluebird.map(
      ...
      async (asset) => {
        ...
        if (!assetType) {
          throw createError(`Unfound asset type (ID ${asset.assetTypeId})`, {
        ...
        transactions = transactions.filter((transaction) =>
          shouldAffectAvailability(transaction, { checkStatus: false })
        )
        ...
        await transaction(knex, async (trx) => {
          ...
          await InternalAvailability.query(trx)
            .delete()
          ...
          await bluebird.map(chunkTransactions, async (chunkTransaction) => {
            const { id, status, startDate, endDate, quantity, assetTypeId } =
              chunkTransaction
  ...
  static async syncInternalAvailabilityTransaction({
    transactionIds,
    platformId,
    env,
  }) {
    const { Transaction, InternalAvailability } = await getModels({
      platformId,
      env,
      models: {
        Transaction: RawTransaction,
    ...
    await bluebird.each(transactions, async (transaction) => {
      const shouldAffect = shouldAffectAvailability(transaction, {
        checkStatus: false,
      })
      ...
      await InternalAvailability.query()
        .patch({
          ...
          unavailable: isStatusBlockingAvailability(
            transaction,
            transaction.status
          ),
        })
  ...
  static async removeInternalAvailability({ assetsIds, platformId, env }) {
    const { InternalAvailability } = await getModels({
      platformId,
      env,
      models: { InternalAvailability: RawInternalAvailability },
    })
